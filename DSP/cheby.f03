...
!                 ss = 0._dp
!                 do i = 0, n
!                     ss = ss + f(i)*cos((pi/(n+1))*j*(i+0.5_dp))
!                 end do
!                 a(j) = 2._dp*ss/(n+1)
!             end do
!             a(0) = 0.5_dp*a(0)
...
!         subroutine chebyderiv(n,a,b,ya,yb) 
!         ! derivative of Chebyshev approximation
!         !   func'([ya,yb]) = sum_{i=0}^(n-1)  b_i T_i
!         ! func  - real function
!         ! n     - number of Chebyshev terms
!         ! a     - expansion coefficients of the function
!         ! b     - expansion coefficients of the derivative
!         ! ya    - lower limit
!         ! yb    - upper limit

!             integer :: n
!             real(dp) :: ya, yb, a(0:maxch), b(0:maxch)
!             integer :: j
    
!             b(n) = 0._dp;   b(n-1) = 2*n*a(n)
!             do j = n-1, 1, -1
!                 b(j-1) = b(j+1) + 2*j*a(j)
!             end do
!             b(0) = b(0)/2
!             b(0:n-1) = b(0:n-1)*2/(yb-ya)

!         end subroutine chebyderiv
            
      
!         function cheby(y,n,a,ya,yb) result(t) 
!         ! calculates the function from the Chebyshev coefficients
!         ! cheby -  func(y) =  sum_{i=0}^n  a_i T_i (x)
!         ! y     - y in [ya,yb]
!         ! n     - number of Chebyshev terms
!         ! a     - expansion coefficients
!         ! ya    - lower limit
...
!         ! finds zero by using Boyd's method: SIAM Review 55 (2) (2013) 375–396.
!         ! n     - number of Chebyshev terms
!         ! a     - expansion coefficients
!         ! ya    - lower limit
!         ! yb    - upper limit
!         ! iz0   - number of zeros found
!         ! z0    - the zeros found
...
!             iz0 = 0
!             do j = 1, n
!                 if( wi0(j) == 0._dp .and. -1 <= wr0(j) .and. wr0(j) <= 1 ) then 
...
!                     real(dp) :: aamat(maxch,maxch), work(lwork), rwork(lwork), vl(1), vr(1)
                    
!                     if (abs(a(n)) == 0._dp) stop 'a(n)=0'
!                     aamat(1:n,1:n) = 0._dp
!                     aamat(1,2) = 1._dp
!                     do j = 2, n-1
!                         aamat(j,j-1) = 0.5_dp;  aamat(j,j+1) = 0.5_dp
!                     end do
!                     aamat(n,1:n) = -a(0:n-1)/(2*a(n))
!                     aamat(n,n-1) = aamat(n,n-1) + 0.5_dp
!                     ie = 0
!                     call dgeev('n','n',n,aamat,maxch,wr,wi,vl,1,vr,1,work,lwork,rwork,ie)                      
...
!         ! locates the zeros of a function by using three-point Pade approximation
!         ! func  - complex function
!         ! zz    - initial guess and final result
!         ! dz    - stepsize
!         ! eps   - the required accuracy
!         ! maxf  - maximum number of function calls 
